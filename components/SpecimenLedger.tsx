  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-[#E4E4E0] pb-6 mb-12">
        ...
        <div className="flex items-center space-x-4 mt-4 sm:mt-0 border border-[#E4E4E0] rounded p-1 bg-[#F1F1EE]">
          <button
            ...
            className={`p-1.5 rounded transition-colors ${viewMode === 'grid' ? 'bg-[#F9F9F7] text-[#1C1C1A] shadow-sm' : 'text-[#767670] hover:text-[#1C1C1A]'}`}
          ...
          <button
            ...
            className={`p-1.5 rounded transition-colors ${viewMode === 'ledger' ? 'bg-[#F9F9F7] text-[#1C1C1A] shadow-sm' : 'text-[#767670] hover:text-[#1C1C1A]'}`}
      ...
      {specimens.length === 0 ? (
        ...
        /* 1. SPECIMEN GRID VIEW */
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
          {specimens.map((item) => (
            <Link 
              key={item.catalogId} 
              href={`/archive/${item.category}/${item.slug}`}
        ...
        /* 2. CLINICAL LEDGER INDEX VIEW */
        <div className="overflow-x-auto border border-[#E4E4E0] bg-[#F9F9F7]">
              ...
              {specimens.map((item) => (
                <tr key={item.catalogId} className="hover:bg-[#F1F1EE] transition-colors group">
                  ...
                  <td className="p-4 font-serif text-sm">
                    <Link href={`/archive/${item.category}/${item.slug}`} className="hover:underline text-[#1C1C1A]">
